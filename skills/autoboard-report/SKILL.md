...
AutoBoard（自动看板）<BOARD_URL> 收集「每个项目当前在排查哪些线上问题、在做哪些 bug/需求、什么状态、
...
curl -s -X POST <BOARD_URL>/api/report -H 'content-type: application/json' -d '{
...
页面 <BOARD_URL> ，全量 JSON：

```bash
curl -s <BOARD_URL>/api/board
```
